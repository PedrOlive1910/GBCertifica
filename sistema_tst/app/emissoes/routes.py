...
def funcionarios_ativos(incluir_id=None):
    consulta = select(Funcionario).order_by(Funcionario.nome)
    if incluir_id:
...
def salvar_emissao(form, emissao=None):
    empresa = db.session.get(Empresa, form.empresa_id.data)
    funcionario = db.session.get(Funcionario, form.funcionario_id.data)
    if not empresa or not funcionario or funcionario.empresa_id != empresa.id:
...
def listar():
    ...
    return render_template(
        ...
        paginacao=paginacao,
        empresas=db.session.scalars(select(Empresa).order_by(Empresa.razao_social)),
...
def editar(emissao_id):
    emissao = db.get_or_404(Emissao, emissao_id)
...
def baixar_arquivo(arquivo_id):
    arquivo = db.get_or_404(ArquivoDocumento, arquivo_id)
