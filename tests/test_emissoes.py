...
from app.services.documentos import localizar_libreoffice, localizar_pdftoppm, renderizar_docx
...
def test_localiza_libreoffice_pelo_env(app, tmp_path, monkeypatch):
    executavel = tmp_path / "soffice.exe"
    executavel.write_bytes(b"teste")
    monkeypatch.setitem(app.config, "LIBREOFFICE_PATH", str(executavel))
    with app.app_context():
        assert localizar_libreoffice() == str(executavel)


def test_localiza_pdftoppm_por_pasta_configurada(app, tmp_path, monkeypatch):
    executavel = tmp_path / "pdftoppm.exe"
    executavel.write_bytes(b"teste")
    monkeypatch.setitem(app.config, "PDFTOPPM_PATH", str(tmp_path))
    with app.app_context():
        assert localizar_pdftoppm() == str(executavel)
